"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DriverRepositoryImpl = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const driver_model_1 = require("./driver.model");
const driver_mapper_1 = require("../mappers/driver.mapper");
let DriverRepositoryImpl = class DriverRepositoryImpl {
    ormRepo;
    constructor(ormRepo) {
        this.ormRepo = ormRepo;
    }
    async save(driver) {
        const entity = driver_mapper_1.DriverMapper.toModel(driver);
        await this.ormRepo.save(entity);
    }
    async findOneById(id) {
        const entity = await this.ormRepo.findOneBy({ id });
        return entity ? driver_mapper_1.DriverMapper.toDomain(entity) : null;
    }
    async findAll() {
        const all = await this.ormRepo.find();
        return all.map(driver_mapper_1.DriverMapper.toDomain);
    }
    async findAvailable() {
        const available = await this.ormRepo.findBy({ status: 'available' });
        return available.map(driver_mapper_1.DriverMapper.toDomain);
    }
    async updateStatus(id, status) {
        await this.ormRepo.update(id, { status });
    }
};
exports.DriverRepositoryImpl = DriverRepositoryImpl;
exports.DriverRepositoryImpl = DriverRepositoryImpl = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(driver_model_1.DriverModel)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], DriverRepositoryImpl);
//# sourceMappingURL=driver.repository.impl.js.map